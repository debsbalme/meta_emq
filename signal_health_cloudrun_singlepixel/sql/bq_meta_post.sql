...
CREATE OR REPLACE TABLE `{bq_project}.{bq_dataset}.pixel_stats_calc`
AS 
SELECT grp.*, 
...
(SELECT
 ...
 from `{bq_project}.{bq_dataset}.pixel_daily_event_agg` evagg
 LEFT OUTER JOIN 
  `{bq_project}.{bq_dataset}.pixel_dataset_quality` dsq
ON 
